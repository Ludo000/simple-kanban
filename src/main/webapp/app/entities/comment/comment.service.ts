...
export class CommentService {
  ...
  protected convertDateFromClient(comment: IComment): IComment {
    const copy: IComment = Object.assign({}, comment, {
      creationDate: comment.creationDate && comment.creationDate.isValid() ? comment.creationDate.toJSON() : undefined,
      modificationDate: comment.modificationDate && comment.modificationDate.isValid() ? comment.modificationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((comment: IComment) => {
        comment.creationDate = comment.creationDate ? moment(comment.creationDate) : undefined;
        comment.modificationDate = comment.modificationDate ? moment(comment.modificationDate) : undefined;
      });
    }
    return res;
  }
